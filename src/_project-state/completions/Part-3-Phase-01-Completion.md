...
Logged in `Decisions.md` as **D-141 … D-145** — renumbered +12 from the branch's original **D-129 … D-133** when this PR merged after 2.06 (which had taken D-129…D-140); see the reconciliation note in `Decisions.md`.

1. **D-141 (orig. D-129) — Executed on the live 2.04 `main`, reconciling the brief's stale "2.06 merged" premise.** `main` is at end of **2.04**; 2.06/2.03 have **not** run and PR #13 does not exist. The scaffolding this phase hardens all exists from 1.05/1.06/2.01, so it is fully executable now. Ran 3.01 ahead of 2.05/2.03 **and** 2.06 at the owner's explicit direction (surfaced + confirmed at kickoff). Every 2.06 reference mapped to the live equivalent (`PROVISIONAL_NORMS`→seed-norms, D-133/134/138/139→D-063/D-071/version-stamps). *Live code wins (CLAUDE.md).*
2. **D-142 — Did not widen the per-item `ResponseTiming`/`CapturedTiming` contract** (D-071's open question); parent-assist + device baseline reach scoring via a new session-level `ScoringContext` on `finalize(state, context?)`. Base case unchanged.
3. **D-143 — Age/assist/device threshold modulation added as NEW labelled seed constants + a pure resolver;** the 1.05 seed VALUES are unchanged. Device-relativity is scoped to the **validity verdict** (in scope); the report's narrative speed-style + the derived attention signal keep the grade-time absolute `tooFast` (report engine is out of scope) — deferred.
4. **D-144 — The on-device progress store uses `localStorage` (a tiny JSON blob), not IndexedDB;** a THIRD store, unjoinable to Store A/B, pure-core / thin-IO split, PII-free by strict schema.
5. **D-145 — The cross-major comparability guard keys on `TASK_BANK_VERSION` major only** and yields a numeric-free fallback (realises [[D-134]]'s flagged carryover).

## Surprises / off-spec changes

- **The brief's context described a repo state that does not exist.** It assumed Phase 2.06 (task bank v2) had merged and referenced `PROVISIONAL_NORMS` and decisions D-133/D-134/D-138/D-139 that are absent from the live tree (`Decisions.md` ends at D-128; `seed-norms.ts` has no `PROVISIONAL_NORMS`/`ATTENTION_BANDS`). Surfaced to the owner before coding; proceeded on the live 2.04 code per their choice. This is the single biggest deviation and the reason the 3.01 decisions were authored as D-129…D-133 (later renumbered +12 → D-141…D-145 when 2.06 merged first; see the reconciliation note in `Decisions.md` and D-146).
...
- `compare.ts` — pure `compareToPrior` + `majorVersion`/`isCrossMajor` (cross-major guard, D-145)
...
**Modified:**
- `src/content/norms/seed-norms.ts` — `resolveValidityThresholds(ctx)` + new modulation seeds (D-143)
- `src/features/scoring/validity.ts` — `computeValidity(items, ctx?)` device-relative + context-modulated
- `src/features/scoring/finalize.ts` — `finalize(state, context?)` + `ScoringContext` (D-142)
...
- `Decisions.md` (D-141…D-145), `src/_project-state/current-state.md`, `src/_project-state/file-map.md`
...
- [x] Session **device baseline** from the first practice item drives a **device-relative** too-fast threshold (§7.2, D-071); two different-baseline devices get comparable relative verdicts (tested). Contract decision made + logged (**not** widened — D-142).
...
- [x] `build` (with env) / `lint` / `typecheck` / `test` / `format:check` pass; new tests per workstream; `current-state.md` + `file-map.md` updated; decisions logged from D-141; PR opened against `main`.
...
- [ ] **Report narrative speed-style + attention are still absolute-thresholded** (grade-time `tooFast`). Making them device-relative belongs with the report-engine phase (out of scope here) — a clean follow-up if desired (D-143).
...
- [ ] **Phase 2.06 (task bank v2) + 2.03 (analytics) + 2.05 (deploy) still un-run** — 3.01 was executed ahead of them (D-141; note: 2.06 has since merged and was reconciled with 3.01 — see D-146). When 2.06 later lands, its `taskBankVersion` **major** bump is exactly what the cross-major guard was built for.

## What's next

Per the owner's sequencing, likely the remaining Part 2 work (2.06 task bank v2, 2.03 analytics, 2.05 deploy) and/or the later Part 3 security pass (rate-limit / anti-bot). Chat should note to Lazar that 3.01 was authored ahead of 2.06/2.03/2.05 but 2.06 merged first, so the log runs 2.06's D-129→D-140 then 3.01's renumbered D-141→D-145; any pending 2.0x decisions continue from D-146.
