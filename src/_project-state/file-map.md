...
**i18n (next-intl 4 — MK at root, SR under `/sr`; Feat-Serbian-Localization):**
- `src/i18n/routing.ts` — **the single canonical locale source**: `defineRouting({ locales:["mk","sr"], defaultLocale:"mk", localePrefix:"as-needed", localeDetection:false })` + `locales`/`defaultLocale`/`AppLocale` exports
- `src/i18n/navigation.ts` — `createNavigation(routing)` → locale-aware `Link`/`redirect`/`usePathname`/`useRouter`/`getPathname` (aliased to a stub under Vitest)
- `src/i18n/metadata.ts` — `alternatesFor(pathname)` (hreflang MK↔SR) + `ogLocaleFor(locale)` (`og:locale` mk_MK/sr_RS); imports `getPathname` via the `@/` path so the Vitest alias applies
- `messages/sr.json` — **full Serbian (Latin) bundle**, exact key + array-shape parity with `mk.json` (parity-tested); brand voice, gender-neutral child copy, latinica program-name transliterations
- `messages/mk.json` — Macedonian strings; +Feat-SR: switcher keys `landing.langSr` + `landing.langSwitcherLabel` added, `landing.langEn` + `landing.langDisabledNote` removed; +1.08: `leadForm` (labels + error tokens), `confirmation`, shared `legal` (verbatim Прилог D.2 data note + D.4 disclaimer), `complete.toForm`; +1.09: `reportPdf` (PDF chrome: wordmark/titles/part banners/section labels/confidence words; reuses `legal`); +1.10: `legal.disclaimerShort` (the §16.1 short line — single source), `pages` (about/privacy/terms copy), `common.home`; **removed** the duplicate `landing.disclaimer` + `prestart.disclaimer` short keys; +2.02: `email` (verbatim Прилог D.3 transactional copy — subject / greeting `{name}` / body / softCta / button / signOff; reuses `legal.disclaimer` = §16.1 placement #5); +2.04: `admin` namespace (internal staff tool, plain MK — login + 2FA, stats labels + gender map, contacts columns + filters + pagination + export); +2.06: `task` namespace reworked for v2 — object-series + 5 new CT-family instructions + EF move counter (`gfSeriesObjects`, `ctLoopEvent`, `ctConditionLoop`, `ctNestedLoop`, `ctCounter`, `ctOptimize`, `efMoves`); removed `ctMaze`/`ctMove*` (maze retired)
- `src/i18n/request.ts` — next-intl request config (+Feat-SR: resolves `requestLocale` + validates via `hasLocale`; loads `messages/${locale}.json`)
- `src/i18n/__tests__/routing.test.ts` — **(Feat-SR)** routing config (locales/default/as-needed/detection-off + `hasLocale`)
- `src/i18n/__tests__/messages-parity.test.ts` — **(Feat-SR)** every enabled locale's bundle has exactly the mk.json key + array shape
...
- `src/app/[locale]/(site)/page.tsx` — **real landing** (1.06): brand hero, value message, real class photo, "Започни проценка" → `/procena`; +1.10: footnote = the shared `<Disclaimer variant="short">` (§16.1 placement #1); +Maint-Logo: real `<Logo />`; **+Feat-SR: async Server Component (`params`/`setRequestLocale` for SSG), locale-aware `Link`/`generateMetadata` (hreflang + og:locale), header now renders the functional `<LocaleSwitcher/>` (MK/SR)**
- `src/app/[locale]/(site)/procena/page.tsx` — assessment route (server); renders the client `Assessment` (1.06)
- `src/app/[locale]/(site)/procena/assessment.tsx` — client flow state machine: setup → pre-start → practice/real (on the 1.05 engine) → completion → **form → confirmation** (1.08, `advanceEndPhase`); finalizes the result once + assembles the report once. **+3.01:** `parentAssistMode` + the device calibration baseline now feed `finalize(state, ctx)`; each finished run's anonymous summary is persisted on-device via `@/features/progress` (repeat ⇒ fresh seed); still no server/PII (1.06/1.08/3.01)
- `src/app/[locale]/(site)/procena/setup-screen.tsx` — age gate 5–13 (<5/>13 blocked, MK message; `noValidate`); no child name (1.06)
- `src/app/[locale]/(site)/procena/prestart-screen.tsx` — instructions + mandatory 5–7 parent screen + confirmation checkbox; +1.10: the shared `<Disclaimer variant="short">` (§16.1 placement #2)
- `src/app/[locale]/(site)/procena/completion-screen.tsx` — "Тестот е завршен" + assembled puzzle-brain + reward badge; +1.08: optional `onProceed` primary button to the lead form
- `src/app/[locale]/(site)/procena/lead-form.tsx` — **lead form (1.08)**: RHF + Zod resolver over the 1.03 primitives; 8 fields (first name only), 3 separate never-pre-ticked consents, inline errors, `form_view` on mount; `CityField` swap-seam; preview seams (`autoValidate`/`defaultValues`); +2.01: passes the real `writeScore` into `runLeadSubmit`
- `src/app/[locale]/(site)/procena/confirmation.tsx` — **confirmation (1.08)**: renders `selectReportSummary` (pentagon + 5 bands + top strength, no number), email-sent line, §D.2 data note, booking CTA (`?grad={city}` + `cta_booking_click`); graceful-retry variant; +1.10: §D.4 = the shared `<Disclaimer variant="full">` (§16.1 placement #3, both branches)
- `src/app/[locale]/(site)/procena/end-phase-view.tsx` — the completion → form → confirmation screen switch (1.08), split out of the flow machine so its guards are unit-testable; +3.02: `LeadForm`/`Confirmation` load via `next/dynamic({ ssr: false })` (lazy-load by section)
...
- `src/app/[locale]/(site)/__tests__/disclaimer-placements.test.tsx` — jsdom (1.10): placement #2 (pre-start) render guard + placement #1 (async landing RSC) source-wiring guard
...
- `locale-switcher.tsx` — **shared functional MK/SR switcher (Feat-SR, D-159)**: `"use client"`; driven off the `@/i18n/routing` locale list; active = check glyph + `aria-current` (not colour-only), ≥44px keyboard target; `useLocale` + `usePathname`/`useRouter().replace(pathname,{locale})`; on the landing + page-shell, NOT in `/procena`
- `band-label.tsx`/`confidence-label.tsx`/`index-band-bar.tsx`/`pentagon.tsx` — **+Feat-SR: locale-aware** via `useLocale` — `BAND_WORDS`/`CONFIDENCE_WORDS` gain an `sr` register (word only; `level`/`bars`/`color` glyph unchanged → PDF sync-guard intact), index name/vertex via `indexLabel(key, locale)` (see `src/lib/indices.ts`)
- `__tests__/locale-switcher.test.tsx` — **(Feat-SR)** jsdom: one control per locale, active `aria-current`, click → `router.replace` (via the navigation stub spy), MK↔SR
...
- `completions/Feat-Serbian-Localization-Completion.md` — **Feat (SR localization)** report (D-159–D-167)

**Test tooling (repo root `test/`):**
- `test/mocks/i18n-navigation.tsx` — **(Feat-SR)** jsdom-safe stub for `@/i18n/navigation` (aliased in `vitest.config.ts`): `Link`→`<a>`, spy-able `router.replace`, `as-needed` `getPathname`

**Feat-SR content/logic touchpoints (SR register added alongside MK — MK output unchanged):**
- `src/content/modules/*.ts` — every module gained an `sr` narrative/activity/CTA/program-name register; `version.ts` `MODULE_LIBRARY_VERSION` 1.0.0→**1.1.0**; `ranges.ts` gained `sr`
- `src/features/report/text.ts` — `{child}` resolver adds „vaše dete“/„Vaše dete“ (neuter); `resolveChild(text, lang)`
- `src/features/report/assemble.ts` — `assembleReport(result, lang="mk")` threads the locale into labels/words/ranges/narrative + the CTA fallback
- `src/lib/indices.ts` — SR label overlay + `indexLabel`/`indexLabelShort(key, locale)` (MK default unchanged)
- `src/features/report/pdf/{document.tsx,render.ts}` — `buildReportDocument`/`renderReportPdf` take `lang`, pick the mk/sr message bundle + locale-aware activity headings
- `src/features/report/pdf/__tests__/fonts.test.ts` — +Feat-SR: latinica glyph coverage (č ć š ž đ) in every weight
- `src/features/report/__tests__/sr-localization.test.ts` — **(Feat-SR)** module completeness + no-MK-leak (5 fixtures) + SR voice-lint + `{child}` resolver
- `src/lib/brevo/email-template.ts` — `buildReportEmail({ …, lang })` picks the mk/sr bundle + `<html lang>`
- `src/app/api/lead/route.ts` — reads the posted `locale`, threads it into assemble/PDF/email + the Brevo `LANGUAGE`
- `src/features/lead/submit.ts` — `submitLead`/`runLeadSubmit` carry the `LeadLocale` (POST body + score-row `language`)
