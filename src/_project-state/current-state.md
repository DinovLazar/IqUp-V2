...
npm test           # Vitest: + disclaimer (component variants / PDF copy-parity / static-page H1s / single-source + placement-wiring guards) — 40 files, 248 tests
...
- [ ] **2 of 7 disclaimer placements wired at their host phase (D-102).** #5 transactional e-mail → **2.02** (Brevo), #7 cookie banner → **3.03**. Copy is already in `messages/mk.json` (`legal.disclaimer` / `legal.disclaimerShort`); each is a one-line `<Disclaimer/>` (or key read) drop-in when the host is built. The plan §14 „all 7 placements" criterion is fully LIVE only at 3.03.
